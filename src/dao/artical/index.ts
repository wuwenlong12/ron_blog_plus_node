...
// 添加文件夹接口
// 添加文件夹或文章接口
export const AddNewFolderOrArticle = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        ...
        if (type === 'folder') {
            // 检查文件夹是否重名
            const existingFolder = await Folder.findOne({ name, parentFolder: parentFolderId });
            if (existingFolder) {
                return res.status(400).json({ code: 1, message: '文件夹名称已存在' });
            }

            // 更新同级 order 值
            await Folder.updateMany({ parentFolder: parentFolderId || null }, { $inc: { order: 1 } });
            ...
            // 更新同级 order 值
            await Article.updateMany({ parentFolder: parentFolderId || null }, { $inc: { order: 1 } });
...
// 更新文章内容接口
export const UpdateArticleContent = async (req: AuthenticatedRequest, res: Response) => {
    const Folder = db.model('Folder');
...
// 修改文件夹名称接口
export const UpdateFolderInfo = async (req: AuthenticatedRequest, res: Response) => {
    const Folder = db.model('Folder');
    const { folderId, newName,newDesc } = req.body;

    if (!folderId || !newName || newName.trim() === '') {
        return res.status(400).json({ code: 1, message: '文件夹 ID 或新名称不能为空' });
    }
    ...
    // 检查新名称是否已经存在（同一父目录下）
    const existingFolder = await Folder.findOne({ name: newName, parentFolder: folderToUpdate.parentFolder });
    if (existingFolder) {
        return res.status(400).json({ code: 1, message: '文件夹名称已存在，请选择其他名称' });
    }
...
export const UpdateItemOrder = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        if (type === 'folder') {
            ...
            // 获取同级文件夹
            const siblingFolders = await Folder.find({ parentFolder: parentFolderId || null }).sort('order');
            ...
            // 获取同级文章
            const siblingArticles = await Article.find({ parentFolder: parentFolderId || null }).sort('order');
